 ...
 * www.napp.dk
 */
var TiDatabase = OS_ANDROID ? Ti.Database : require('ti.advanceddatabase');
...
function Sync(method, model, opts) {
    ...
    switch (method) {
        ...
        case 'read':
            ...
            if (DEBUG) {
                //Ti.API.timestamp("[REST API] options: ");
                Ti.API.info(params.urlparams);
                //Ti.API.timestamp("[SQL API] options: ");
                Ti.API.info(params.sql);
            }
            ...
            apiCall(params, function(_response) {
                if (_response.success) {
                    ...
                    Ti.API.info("resp",_response.responseData);
                    var data = parseJSON(_response, parentNode);
                    
                    //TODO: remove this hack
                    var total = JSON.parse(_response.responseText).meta ? JSON.parse(_response.responseText).meta.total : 0;
                    Ti.App.fireEvent("search:updateTotal", {total: total});
                    //******************

                    if (data.error || (model.config.adapter.requiredStatus && model.config.adapter.requiredStatus != data.status)) {
                        _.isFunction(params.error) && params.error(data);
                    } else {
                        //Ti.API.timestamp('PARSED JSON');
                        if (!params.remoteOnly && !params.localOnly) {//we dont want to manipulate the data on localOnly requests
                            //Ti.API.timestamp('START SAVE SQL');
                            saveData(data);
                        }
                        //Ti.API.timestamp('START READ SQL');
                        if (params.remoteOnly) {
                            if (isCollection && _.isArray(data) && data.length > 1) {
                                //push the models
                                resp = [];
        ...
        try {
            if (!_.isArray(data)) {// its a model
                if (!_.isUndefined(data["is_deleted"])) {//delete item
                    ...
                    var data = updateSQL(data);
                    //item exists - update it
                    //commitTransaction();
                    db.execute('COMMIT;');
                    return data;
                } else {
                    var data = createSQL(data);
    ...
    function readSQL(data) {
        ...
        if (!isCollection && data && data[model.idAttribute]) {
            if (!opts.sql)
                opts.sql = {};
            if (!opts.sql.where)
                opts.sql.where = {};
            opts.sql.where[model.idAttribute] = data[model.idAttribute]
        }
        if (!isCollection && opts && opts.findBySerial) {
            if (!opts.sql)
                opts.sql = {};
            if (!opts.sql.where)
                opts.sql.where = {};
            //opts.sql.where[model.idAttribute] = data[model.idAttribute]
            //TODO: add read by serial (find ID by serial and add ID to where)
        ...
        if (opts.query) {
            ...
            if (DEBUG) {
                Ti.API.info("[SQL REST API] SQL QUERY: " + sql);
            }
